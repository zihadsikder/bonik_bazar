...
class FetchBlogsCubit extends Cubit<FetchBlogsState> {
  ...
  Future<void> fetchBlogs() async {
    try {
      ...
      DataOutput<BlogModel> result =
          await _blogRepository.fetchBlogs(page: 1);
  ...
  Future<void> fetchBlogsMore() async {
    try {
      if (state is FetchBlogsSuccess) {
        ...
        DataOutput<BlogModel> result =
            await _blogRepository.fetchBlogs(
          page: (state as FetchBlogsSuccess).page+1,
        );

        FetchBlogsSuccess blogModelState =
            (state as FetchBlogsSuccess);
